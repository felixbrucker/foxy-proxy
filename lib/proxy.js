...
class Proxy {
  ...
  async submitNonce(submissionObj, options) {
    ...
    if (!this.miners[minerId].accountIds.find(submission.accountId)) {
    ...
    if (submission.deadline.isNaN()) {
      ...
      if (result.error) {
        eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
      ...
      if (realResult.result === 'success') {
        eventBus.publish('log/info', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
    ...
    if (result.error) {
      eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
          `account=${outputUtil.getString(submission.accountId, upstream.accountColor)} | Error: tried submitting DL ${outputUtil.getString(adjustedDL.toString(), upstream.deadlineColor)}, failed with error ` +
    ...
    if (realResult.result === 'success') {
      eventBus.publish('log/info', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
          `account=${outputUtil.getString(submission.accountId, upstream.accountColor)} | submitted DL ${outputUtil.getString(adjustedDL.toString(), upstream.deadlineColor)}`);
    } else {
      eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
          `account=${outputUtil.getString(submission.accountId, upstream.accountColor)} | Error: tried submitting DL ${outputUtil.getString(adjustedDL.toString(), upstream.deadlineColor)}, failed with error ` +
