...
class Proxy {
  ...
  async submitNonce(submissionObj, options) {
    ...
    if (!submission.isValid()) {
      eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${minerId} | Error: tried submitting invalid data`);
    ...
    if (!upstream) {
      eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${minerId} | Error: tried submitting DL for a different round`);
    ...
    if (submission.deadline.isNaN()) {
      ...
      if (result.error) {
        eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${minerId} | ` +
      ...
      if (realResult.result === 'success') {
        eventBus.publish('log/info', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${minerId} | ` +
