...
class Proxy {
  ...
  async handleSubmitNonce(ctx) {
    ...
    if (result.error) {
      console.error(`${moment().format('YYYY-MM-DD HH:mm:ss.SSS')} | ${this.proxyConfig.name} | ${upstream.upstreamConfig.name} | ${minerId} | account=${submission.accountId} | tried submitting DL ${adjustedDL.toString()}, failed`);
      ctx.status = 400;
      ctx.body = {
        error: result.error,
      };
    ...
    if (realResult.result === 'success') {
      console.log(`${moment().format('YYYY-MM-DD HH:mm:ss.SSS')} | ${this.proxyConfig.name} | ${upstream.upstreamConfig.name} | ${minerId} | account=${submission.accountId} | submitted DL ${adjustedDL.toString()}`);
    } else {
      console.error(`${moment().format('YYYY-MM-DD HH:mm:ss.SSS')} | ${this.proxyConfig.name} | ${upstream.upstreamConfig.name} | ${minerId} | account=${submission.accountId} | tried submitting DL ${adjustedDL.toString()}, failed`);
    }
