...
class Proxy {
  ...
  async submitNonce(submissionObj, options) {
    ...
    if (submission.deadline.isNaN()) {
      ...
      if (result.error) {
        eventBus.publish('log/error', `${this.proxyConfig.name} | ${upstream.upstreamConfig.name} | ${minerId} | account=${submission.accountId} | Error: tried submitting client side solo submission with nonce ${submission.nonce.toString()}, failed`);
        return {
          error: result.error,
        };
      }
      ...
      if (realResult.result === 'success') {
        eventBus.publish('log/info', `${this.proxyConfig.name} | ${upstream.upstreamConfig.name} | ${minerId} | account=${submission.accountId} | submitted client side solo submission with nonce ${submission.nonce.toString()}`);
      } else {
        eventBus.publish('log/error', `${this.proxyConfig.name} | ${upstream.upstreamConfig.name} | ${minerId} | account=${submission.accountId} | Error: tried submitting client side solo submission with nonce ${submission.nonce.toString()}, failed`);
      }
