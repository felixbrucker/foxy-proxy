...
class Proxy {
  ...
  async submitNonce(submissionObj, options) {
    ...
    if (result.error) {
      eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
          `account=${outputUtil.getString(submission.accountId, upstream.accountColor)} | Error: tried submitting DL ${outputUtil.getString(this.getFormattedDeadline(adjustedDL), upstream.deadlineColor)}, failed with error ` +
    ...
    if (realResult.result === 'success') {
      eventBus.publish('log/info', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
          `account=${outputUtil.getString(submission.accountId, upstream.accountColor)} | submitted DL ${outputUtil.getString(this.getFormattedDeadline(adjustedDL), upstream.deadlineColor)}`);
    } else {
      eventBus.publish('log/error', `${outputUtil.getName(this.proxyConfig)} | ${outputUtil.getName(upstream.upstreamConfig)} | ${outputUtil.getString(minerId, upstream.minerColor)} | ` +
          `account=${outputUtil.getString(submission.accountId, upstream.accountColor)} | Error: tried submitting DL ${outputUtil.getString(this.getFormattedDeadline(adjustedDL), upstream.deadlineColor)}, failed with error ` +
