...
class Proxy {
  ...
  async init() {
    ...
    this.upstreams = await Promise.all(this.proxyConfig.upstreams
      .filter(upstreamConfig => !upstreamConfig.disabled)
      .map(async (upstreamConfig, index) => {
        ...
        upstream.on('new-round', (miningInfo) => {
          if (this.proxyConfig.useProfitability) {
            ...
            eventBus.publish('log/debug', `Profitability-Service | ${upstream.fullUpstreamNameLogs} | Set weight ${upstream.prio}`);
          }
  ...
  async submitNonce(submissionObj, options) {
    ...
    for (let upstream of upstreams) {
      ...
      if (!targetDL && upstream.useSubmitProbability) {
        ...
        upstream.dynamicTargetDL = targetDL;
      }
      ...
      if (realResult.height && submission.height !== realResult.height) {
        returnValues.push({
          error:  {
      ...
      if (realResult.deadline !== adjustedDL.toNumber()) {
        eventBus.publish('log/error', `${fullUpstreamNameLogs} | ${outputUtil.getString(minerId, miner.color || upstream.minerColor)} | ` +
