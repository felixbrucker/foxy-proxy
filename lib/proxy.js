...
class Proxy {
  ...
  constructor(upstreamConfig) {
    ...
    this.alphas = Proxy.calculateAlphas(this.historicalRoundsToKeep, 3);
  }
  ...
  async getEstimatedCapacity() {
    ...
    const weightedDLSum = roundsWithDLs
      .map(round => BigNumber(round.bestDL).multipliedBy(round.baseTarget))
    ...
    return this.alphas[nConf - 1] * blockTime * (nConf - 1) / (weightedDLSum.dividedBy(blockZeroBaseTarget).toNumber());
  }
  ...
  async handleSubmitNonce(ctx) {
    ...
    if (result.error) {
      console.error(`${new Date().toISOString()} | ${this.upstreamConfig.name} | ${minerId} | account=${minerRound.accountId} | tried submitting DL ${adjustedDL.toString()}, failed`);
      ctx.status = 400;
      ctx.body = {
        error: result.error,
      };
    ...
    if (realResult.result === 'success') {
      console.log(`${new Date().toISOString()} | ${this.upstreamConfig.name} | ${minerId} | account=${minerRound.accountId} | submitted DL ${adjustedDL.toString()}`);
    } else {
      console.error(`${new Date().toISOString()} | ${this.upstreamConfig.name} | ${minerId} | account=${minerRound.accountId} | tried submitting DL ${adjustedDL.toString()}, failed`);
    }
