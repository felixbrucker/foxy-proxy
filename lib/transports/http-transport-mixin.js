    ...
    switch (requestType) {
      ...
      case 'submitNonce':
        const options = {
          ...
          minerAlias: ctx.req.headers['x-accountalias'] || ctx.req.headers['x-mineralias'] || null,
          color: ctx.req.headers['x-color'] || null,
        };
        if (options.minerName) {
          options.minerName = decodeURI(options.minerName);
        }
        if (options.minerAlias) {
          options.minerAlias = decodeURI(options.minerAlias);
        }
        const submissionObj = {
