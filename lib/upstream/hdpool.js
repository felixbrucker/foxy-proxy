...
class HDPool extends statsMixin(estimatedCapacityMixin(EventEmitter)) {
  ...
  async setupMinersForAccount(account, doHandlerSetup) {
    for (let [minerName, miner] of account.miners) {
      ...
      await client.init();
      client.subscribe('websocket/opened', () => eventBus.publish('log/debug', `${this.fullUpstreamNameLogs} | miner=${minerName} | websocket opened`));
      client.subscribe('websocket/closed', () => eventBus.publish('log/debug', `${this.fullUpstreamNameLogs} | miner=${minerName} | websocket closed`));
  ...
  async onNewRound(para) {
    ...
    if (lastBlockHeight !== 0) {
      ...
      const roundWon = lastBlockWinner === -1 ? null : accountIds.some(accountId => accountId === lastBlockWinner);
