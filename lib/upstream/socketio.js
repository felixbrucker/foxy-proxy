...
class SocketIo extends BaseUpstream {
  constructor(upstreamConfig, miners, proxyConfig) {
    ...
    this.historicalRoundsToKeep = this.upstreamConfig.historicalRoundsToKeep || (this.isBHD ? 288 : 360);
  ...
  async setupMinersForUrl(urlMapping, doHandlerSetup) {
    ...
    client.emit('getMiningInfo', this.onNewRound.bind(this));
  }
  ...
  async getBlockWinnerAccountId(height) {
    ...
    return util.getBlockWinnerAccountId(this.upstreamConfig.walletUrl, this.isBHD, height, this.upstreamConfig.customEndpoint); // TODO: query upstream
  }
