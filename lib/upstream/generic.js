  ...
  async updateMiningInfo() {
    ...
    try {
      ...
      if (isBHDSolo) {
        ...
        request = superagent.get(`${this.upstreamConfig.url}/${endpoint}`).set('User-Agent', `Burst-BHD-Proxy ${version}`);
      }

      if (this.upstreamConfig.accountKey) {
        request = request.set('X-Account', this.upstreamConfig.accountKey).set('X-MinerName', `Burst-BHD-Proxy ${version}/${hostname()}`);
      }
