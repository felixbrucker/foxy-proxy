...
class GenericUpstream extends BaseUpstream {
  ...
  async updateMiningInfo() {
    ...
    try {
      ...
      let newBlockLine = `${this.fullUpstreamNameLogs} | ${outputUtil.getString(`New block ${miningInfo.height}, baseTarget ${miningInfo.baseTarget}, netDiff ${miningInfo.netDiff.toFixed(0)} TB`, 'green')}`;
  ...
  async submitNonce(submission, minerSoftware, options) {
    ...
    try {
      ...
      let {text: result} = await request.retry(5, (err) => {
        ...
        eventBus.publish('log/debug', `${this.fullUpstreamNameLogs} | Error: Failed submitting DL ${submission.deadline.toString()}, retrying ..`);
        return true;
      });
      result = JSON.parse(result);
