...
class Upstream {
    ...
    try {
      ...
      if (miningInfo.height > this.upstream.miningInfo.height) {
        ...
        Object.keys(this.upstream.miners).forEach(key => {
          if (miningInfo.height - 10 < this.upstream.miners[key].lastBlockSubmitted) {
            return;
          }
  ...
  async getBlockWinnerAccountId(height) {
    if (this.isBHD) {
      ...
      return block.minerAccountId.toString();
    } else {
  ...
  async handleSubmitNonce(ctx) {
    ...
    this.upstream.miners[minerId].lastBlockSubmitted = minerRound.height;
    ...
    if (this.upstream.mode === 'pool') {
      ...
      const passphrase = this.upstream.passphrases[minerRound.accountId];
      if (!passphrase) {
        ctx.status = 400;
        ctx.body = {
          error: {
    ...
    try {
      let {text: result} = await superagent.post(`${this.upstream.url}/burst`).query(queryParams).unset('User-Agent').retry(2);
      result = JSON.parse(result);
