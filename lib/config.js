const fs = require('fs');
const JSONbig = require('json-bigint');
const mkdirp = require('mkdirp');
const moment = require('moment');
const os = require('os');
const path = require('path');
const YAML = require('js-yaml');

module.exports = class Config {
  static get defaultConfig() {
    return {
      proxies: [{
        name: 'Burst-BHD',
        maxScanTime: 35,
        upstreams: [{
          name: 'Burst Solo',
          url: 'http://localhost:8125',
          mode: 'solo',
          passphrase: 'asdf',
          targetDL: 86400,
          prio: 10,
        }, {
          name: 'HDPool',
          mode: 'pool',
          type: 'hdpool',
          accountKey: '1234',
          targetDL: 31536000,
          prio: 11,
          sendMiningSoftwareName: true,
        }],
      }],
      listenAddr: '127.0.0.1:12345',
      useMultiplePorts: false,
    };
  }

  static detectFilePath(fileName) {
    if (fs.existsSync(fileName)) {
      return fileName;
    }

    return path.join(os.homedir(), '.config/bhd-burst-proxy', fileName);
  }

  constructor() {
    this.filePath = Config.detectFilePath('config.yaml');
    this.ensureFilePathExists();
    this.migrateFromJSON();
    this.loadFromFile();
    this.migrateToProxyConfig();
  }

  ensureFilePathExists() {
    const dirPath = path.dirname(this.filePath);
    if (dirPath === '.') {
      return;
    }
    mkdirp.sync(dirPath);
  }

  migrateFromJSON() {
    if (!fs.existsSync('config.json')) {
      return;
    }
    console.log(`${moment().format('YYYY-MM-DD HH:mm:ss.SSS')} | old config file format detected, migrating to yaml ..`);
    const oldConfig = JSONbig.parse(fs.readFileSync('config.json'));
    fs.writeFileSync(this.filePath, YAML.safeDump(oldConfig));
    fs.unlinkSync('config.json');
  }

  migrateToProxyConfig() {
    if (!this.config.upstreams) {
      return;
    }
    this.config.proxies = this.config.upstreams.map(upstreamConfig => ({
      name: upstreamConfig.name,
      upstreams: [upstreamConfig]
    }));
    delete this.config.upstreams;
    this.saveToFile();
  }

  loadFromFile() {
    let file;
    try {
      file = fs.readFileSync(this.filePath);
    } catch (err) {
      console.log(`${moment().format('YYYY-MM-DD HH:mm:ss.SSS')} | first start detected, creating the config file (${this.filePath}), please adjust it to your preferences`);
      this.initFromObject();
      this.saveToFile();
      process.exit(0);
    }
    const configObject = YAML.safeLoad(file);
    this.initFromObject(configObject);
  }

  saveToFile() {
    const yaml = YAML.safeDump(this.config);
    fs.writeFileSync(this.filePath, yaml);
  }

  initFromObject(configObject = null) {
    this._config = configObject || Config.defaultConfig;
  }

  get useMultiplePorts() {
    return this.config.useMultiplePorts;
  }

  get proxies() {
    return this.config.proxies;
  }

  get listenAddr() {
    return this.config.listenAddr;
  }

  get listenHost() {
    return this.config.listenAddr.split(':')[0];
  }

  get listenPort() {
    return parseInt(this.config.listenAddr.split(':')[1], 10);
  }

  get config() {
    return this._config;
  }
};
