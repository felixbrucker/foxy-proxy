const fs = require('fs');
const JSONbig = require('json-bigint');

module.exports = class Config {
  static get defaultConfig() {
    return {
      upstreams: [{
        name: 'Burst Solo',                     // name has to be unique
        url: 'http://localhost:8125',
        mode: 'solo',
        passphrases: {
          '12345': 'asdf',
        },
        passphrase: 'asdf',                     // set this passphrase for all accounts
        targetDL: 86400,
      }, {
        name: 'BHD Solo',
        url: 'http://localhost:8732',
        mode: 'solo',
        passphrases: {
          '12345': 'asdf',
        },
        targetDL: 86400,
        accountIdToUrl: {
          '12345': 'http://localhost:8732',
        },
      }, {
        name: 'BHD HDPool',
        url: 'http://localhost:60100',            // Use their hdproxy for now, custom websockets ahead
        walletUrl: 'http://localhost:8732',
        mode: 'pool',
        targetDL: 31536000,
      }, {
        name: 'BHD HPool',
        url: 'https://bhd.hpool.com',
        walletUrl: 'http://localhost:8732',
        mode: 'pool',
        accountKey: '1234',
        targetDL: 86400,
        updateMiningInfoInterval: 5000,           // in ms, default is 1000 ms
      }],
      listenAddr: '127.0.0.1:12345',
      useMultiplePorts: false,                    // enable for blago
    };
  }

  constructor(filePath) {
    this.filePath = filePath;
    this.loadFromFile();
  }

  loadFromFile() {
    let file;
    try {
      file = fs.readFileSync(this.filePath);
    } catch (err) {
      this.initFromObject();
      this.saveToFile();
      file = fs.readFileSync(this.filePath);
    }
    const configObject = JSONbig.parse(file);
    this.initFromObject(configObject);
  }

  saveToFile() {
    const json = JSONbig.stringify(this.config, null, 2);
    fs.writeFileSync(this.filePath, json);
  }

  initFromObject(configObject = null) {
    this._config = configObject || Config.defaultConfig;
  }

  get useMultiplePorts() {
    return this.config.useMultiplePorts;
  }

  get upstreams() {
    return this.config.upstreams;
  }

  get listenAddr() {
    return this.config.listenAddr;
  }

  get listenHost() {
    return this.config.listenAddr.split(':')[0];
  }

  get listenPort() {
    return parseInt(this.config.listenAddr.split(':')[1], 10);
  }

  get config() {
    return this._config;
  }
};
